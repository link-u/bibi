...
Bibi.initialize = () => {
    ...
    // sqex: 文言を変える
    I.note(`<span class="non-visual">Loading...</span>`);
    U.initialize();
    // Book Style
...
Bibi.loadBook = (BookDataParam) => Promise.resolve().then(() => {
    ...
    if(S['use-cookie']) {
        ...
        // sqex: 読み返し時に表示位置を記憶せず1ページ目に遷移させる
...
L.loadContainer = () => O.openDocument(B.Container).then(L.loadContainer.process);
    ...
    L.loadContainer.process = (Doc) => {
        // sqex: 読み込みに失敗した際に端末にerrorのEventを返す
        try {
